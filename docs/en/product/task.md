...
permalink: /en/product/o3fce1l0/
---

# Task

Tasks are the most fundamental data in the entire application.
...
All task operations are located in the task toolbar. Click on the blank area of a task or the more icon on the right to view them.
...
- Manage manual task views for this task
- Set recurring task
...
#### Edit Subtasks via Subtask Editor Tool

Find the "Edit Subtasks" icon in the task toolbar and open the subtask editor drawer
...
Task descriptions support markdown syntax.
...
There are two ways to add images to task descriptions. After adding, the image will be saved to the user data folder and referenced using markdown syntax:

- Paste from clipboard, e.g., after taking a screenshot, use `ctrl + v` directly in the content.
...
Hover over the tool icon to see a quick time setting dropdown, or click the icon directly to customize
...
Workload represents the weight of a task. When calculating subtask completion progress, task workload is used for weighted average calculations. The default task workload is 1, so if unmodified, the completion progress calculation will be consistent with the weighted value of task count.

Hover over the tool icon to see a quick workload setting dropdown, or click the icon to customize. For tasks with set workloads, you can edit by clicking the workload in the task description. Enter the default workload to remove the indicator.
...
Hover over the tool icon to see a quick priority setting dropdown, or click the icon to customize. For tasks with set priorities, you can edit by clicking the priority in the task description. Enter the default priority to remove the indicator.
...
There are multiple ways to change a task's parent task or task group:
...
### Edit Task Recurring Settings

See [Recurring Tasks](/product/nexttask)
...
:::warning
Deletion is irreversible and will have a confirmation prompt.
:::
